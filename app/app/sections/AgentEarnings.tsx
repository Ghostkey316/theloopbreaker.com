            ...
            <div style={{ display: "flex", gap: 10, flexDirection: isMobile ? "column" : "row" }}>
              ...
              <button
                onClick={async () => {
                  ...
                  setWithdrawing(true);
                  try {
                    // Real withdrawal via AIAccountabilityBondsV2
                    showToast("Scanning bonds on Base, Avalanche, Ethereum...", "info");
                    const result = await withdrawAllBonds((step: WithdrawalStep) => {
                      const icon = step.status === 'completed' ? '✅' : step.status === 'failed' ? '❌' : '⏳';
                      showToast(`${icon} ${step.description}`, step.status === 'failed' ? 'warning' : 'info');
                    });
                    if (result.success) {
                      if (result.totalWithdrawn !== '0') {
                        showToast(`Withdrawal complete! ${result.totalWithdrawn} ETH distributed.`, 'success');
                      } else if (result.error) {
                        showToast(result.error, 'info');
                      } else {
                        showToast('Distribution requested. Come back after timelock to claim.', 'success');
                      }
                      loadData();
                    } else {
                      showToast(result.error || 'No bonds available for withdrawal.', 'info');
                    }
                  } catch (e) {
              ...
              >
                {withdrawing ? "Processing..." : stats.availableBalance > 0 ? "Withdraw All" : "Check / Request Distribution"}
              </button>
            </div>
          </div>
