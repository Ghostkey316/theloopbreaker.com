...
import { getWalletAddress, isWalletUnlocked } from "../lib/wallet";
...
interface TrustMetrics {
  ...
  trustScore: number;
  grade: string;
  // Real on-chain trust data
  onChainData?: TrustScoreBreakdown;
  isOnChain?: boolean;
}
...
export default function TrustScore() {
  ...
  const loadMetrics = async () => {
    ...
    try {
      ...
      // Trust score is calculated from REAL on-chain data.
      // Use trust-engine to fetch real data from contracts
      let onChainResult: TrustScoreBreakdown | null = null;
      let isOnChain = false;

      try {
        onChainResult = await calculateTrustScore();
        isOnChain = onChainResult.isLive;
      } catch (err) {
        console.warn('[TrustScore] On-chain trust engine failed, using fallback:', err);
      }

      // If we got real on-chain data, use it; otherwise fall back to contract health check
      let score = 0;
      if (onChainResult) {
        score = onChainResult.score;
      } else {
        // Fallback: calculate from contract health + registration
        score += (verifiedCount / totalCount) * 50;
        score += (chainsActive / 2) * 20;
        if (userReg) score += 15;
        if (regChains.length >= 2) score += 15;
        else if (regChains.length >= 1) score += 8;
        score = Math.round(Math.min(score, 100));
      }

      const grade = onChainResult?.grade ?? (
      ...
      setMetrics({
        ...
        trustScore: score,
        grade,
        onChainData: onChainResult ?? undefined,
        isOnChain,
      });
    } catch {
      setMetrics({
    ...
    <div className="page-enter" style={{ padding: isMobile ? "24px 16px 48px" : "48px 40px", maxWidth: 680, margin: "0 auto" }}>
      ...
      {metrics && !loading && (
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
            <h2 style={{ fontSize: 11, fontWeight: 600, color: "#71717A", textTransform: "uppercase", letterSpacing: "0.1em", margin: 0 }}>Score Breakdown</h2>
            {metrics.isOnChain && (
              <span style={{ fontSize: 9, fontWeight: 700, color: '#22C55E', backgroundColor: 'rgba(34,197,94,0.10)', padding: '2px 6px', borderRadius: 4, textTransform: 'uppercase', letterSpacing: '0.06em' }}>On-Chain</span>
            )}
          </div>
          {metrics.onChainData ? (
            // Real on-chain breakdown from trust-engine
            metrics.onChainData.components.map((comp: TrustComponent) => (
              <div key={comp.name} style={{ marginBottom: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  <div>
                    <span style={{ fontSize: 13, color: "#A1A1AA", lineHeight: 1.5 }}>{comp.name}</span>
                    <span style={{ fontSize: 10, color: '#52525B', marginLeft: 8 }}>{comp.source}{comp.verified ? ' ✓' : ''}</span>
                  </div>
                  <span style={{ fontSize: 12, color: "#71717A", ...monoStyle }}>{comp.score}/{comp.maxScore}</span>
                </div>
                <div style={{
                  ...
                  <div style={{
                    width: `${comp.maxScore > 0 ? (comp.score / comp.maxScore) * 100 : 0}%`, height: "100%",
                    backgroundColor: comp.score >= comp.maxScore * 0.8 ? "#22C55E" : comp.score >= comp.maxScore * 0.5 ? "#EAB308" : "#EF4444",
                    borderRadius: 2,
      ...
      <div style={{
        ...
        <p style={{ fontSize: 12, color: '#A1A1AA', lineHeight: 1.8, marginBottom: 6 }}>
          Trust score is calculated from <strong style={{ color: '#F4F4F5' }}>real on-chain data</strong>: bond amounts from AIAccountabilityBondsV2,
          registration status from ERC-8004, VNS identity, and contract health verified via BaseScan/Snowtrace APIs.
          {metrics?.isOnChain ? ' Your score reflects live blockchain state.' : ' Connect your wallet for personalized on-chain scoring.'}
        </p>
