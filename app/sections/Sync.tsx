...
export default function Sync() {
  ...
  return (
    <div style={{ padding: isMobile ? '20px 16px 40px' : '32px 32px', maxWidth: 680, margin: '0 auto' }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: isMobile ? 20 : 24, fontWeight: 700, color: '#FFFFFF', marginBottom: 4, letterSpacing: '-0.03em' }}>Data Sync</h1>
        <p style={{ fontSize: isMobile ? 12 : 13, color: '#A0A0A8', lineHeight: 1.6, letterSpacing: '-0.01em' }}>Export and import your Vaultfire data. All data is stored locally in your browser.</p>
      </div>
      ...
      <div style={{
        ...
        {statItems.map((stat) => (
          <div key={stat.label} style={{
            ...
            <p style={{ fontSize: 9, color: '#666670', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4, fontWeight: 500 }}>{stat.label}</p>
            <p style={{
              fontSize: isMobile ? 15 : 16, fontWeight: 700, color: stat.color,
      ...
      {stats.lastSync && (
        <div style={{
          ...
          <p style={{ fontSize: 11, color: '#22C55E' }}>Last export: {new Date(stats.lastSync).toLocaleString()}</p>
        </div>
      )}
      ...
      <div style={{
        ...
        <h2 style={{ fontSize: 13, fontWeight: 600, color: '#FFFFFF', marginBottom: 3, letterSpacing: '-0.01em' }}>Export Data</h2>
        <p style={{ fontSize: 11, color: '#A0A0A8', marginBottom: 12, lineHeight: 1.6, letterSpacing: '-0.01em' }}>
        ...
        <button onClick={handleExport}
          style={{
            ...
            padding: '8px 18px',
            background: exportSuccess ? '#22C55E' : 'linear-gradient(135deg, #F97316, #EA6C0A)',
            border: 'none', borderRadius: 8, color: '#0A0A0C',
            fontSize: 12, fontWeight: 600, cursor: 'pointer',
            width: isMobile ? '100%' : 'auto',
            transition: 'all 0.2s ease', letterSpacing: '-0.01em',
            boxShadow: '0 2px 12px rgba(249,115,22,0.2)',
          }}>
          {exportSuccess ? <><CheckCircleIcon size={12} /> Exported!</> : <><DownloadIcon size={12} /> Export Backup</>}
        </button>
      </div>
      ...
      <div style={{
        ...
        <h2 style={{ fontSize: 13, fontWeight: 600, color: '#FFFFFF', marginBottom: 3, letterSpacing: '-0.01em' }}>Import Data</h2>
        <p style={{ fontSize: 11, color: '#A0A0A8', marginBottom: 12, lineHeight: 1.6, letterSpacing: '-0.01em' }}>
          Restore from a previously exported backup file. This will merge with your existing data.
        </p>
        {importError && (
          <div style={{ marginBottom: 10, padding: '7px 10px', backgroundColor: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.12)', borderRadius: 8 }}>
            <p style={{ fontSize: 11, color: '#EF4444' }}>{importError}</p>
          </div>
        )}
        {importSuccess && (
          <div style={{ marginBottom: 10, padding: '7px 10px', backgroundColor: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.12)', borderRadius: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
            <CheckCircleIcon size={11} />
            <p style={{ fontSize: 11, color: '#22C55E' }}>Data imported successfully!</p>
          </div>
        )}
        <label style={{
          display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6,
          padding: '8px 18px', backgroundColor: 'rgba(255,255,255,0.04)',
          border: '1px solid rgba(255,255,255,0.06)', borderRadius: 8,
          color: '#FFFFFF', fontSize: 12, cursor: 'pointer', textAlign: 'center',
          width: isMobile ? '100%' : 'auto',
      ...
      <div style={{
        ...
        <h2 style={{ fontSize: 13, fontWeight: 600, color: '#EF4444', marginBottom: 3, letterSpacing: '-0.01em' }}>Clear All Data</h2>
        <p style={{ fontSize: 11, color: '#A0A0A8', marginBottom: 12, lineHeight: 1.6, letterSpacing: '-0.01em' }}>
