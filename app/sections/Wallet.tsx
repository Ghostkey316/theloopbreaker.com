...
export default function Wallet() {
  ...
  if (view === "none") {
    return (
      <div style={{ padding: isMobile ? '16px 16px 32px' : 32, maxWidth: 480, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: isMobile ? 24 : 32 }}>
          <div style={{ fontSize: isMobile ? 40 : 48, marginBottom: 12 }}>💼</div>
          <h1 style={{ fontSize: isMobile ? 22 : 24, fontWeight: 700, color: "#ECEDEE", marginBottom: 8 }}>Vaultfire Wallet</h1>
          <p style={{ fontSize: isMobile ? 13 : 14, color: "#9BA1A6", lineHeight: 1.6 }}>Create or import an Ethereum-compatible wallet. Keys stored locally in your browser.</p>
        </div>
  ...
  if (view === "import-mnemonic" || view === "import-pk") {
    ...
    return (
      <div style={{ padding: isMobile ? '16px 16px 32px' : 32, maxWidth: 480, margin: "0 auto" }}>
        ...
        <h2 style={{ fontSize: isMobile ? 18 : 20, fontWeight: 600, color: "#ECEDEE", marginBottom: 8 }}>{isMnemonicView ? "Import Seed Phrase" : "Import Private Key"}</h2>
        <p style={{ fontSize: 13, color: "#9BA1A6", marginBottom: 20 }}>{isMnemonicView ? "Enter your 12 or 24 word seed phrase." : "Enter your private key (with or without 0x prefix)."}</p>
  ...
  return (
    <div style={{ padding: isMobile ? '16px 16px 32px' : 32, maxWidth: 600, margin: "0 auto" }}>
      {/* Header */}
      <div style={{
        display: "flex",
        alignItems: "center",
        ...
        <h1 style={{ fontSize: isMobile ? 20 : 22, fontWeight: 700, color: "#ECEDEE" }}>My Wallet</h1>
      ...
      <div style={{ backgroundColor: "#1A1A1E", border: "1px solid #2A2A2E", borderRadius: 14, padding: isMobile ? 16 : 20, marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <button onClick={() => copyToClipboard(walletData?.address || "", "address")}
            style={{ flexShrink: 0, padding: "6px 12px", backgroundColor: copied === "address" ? "#22C55E20" : "#2A2A2E", border: "none", borderRadius: 8, color: copied === "address" ? "#22C55E" : "#9BA1A6", fontSize: 12, cursor: "pointer", whiteSpace: "nowrap" }}>
            {copied === "address" ? "✓" : "Copy"}
          </button>
        </div>
      </div>
            ...
            {balances.map((bal) => (
              <div key={bal.chain} style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                  <div style={{ width: 8, height: 8, borderRadius: "50%", backgroundColor: bal.color, flexShrink: 0 }} />
                ...
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  <p style={{ fontSize: isMobile ? 13 : 14, fontWeight: 600, color: "#ECEDEE", fontFamily: "monospace" }}>{bal.balanceFormatted}</p>
            ...
            <div>
              <div style={{
                ...
                {walletData.mnemonic.split(" ").map((word, i) => (
                  <div key={i} style={{ padding: "6px 10px", backgroundColor: "#0A0A0C", border: "1px solid #2A2A2E", borderRadius: 6, fontSize: isMobile ? 11 : 12, color: "#ECEDEE", fontFamily: "monospace", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
