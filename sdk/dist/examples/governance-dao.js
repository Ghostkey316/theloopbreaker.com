...
exports.reputationWeightedVoting = reputationWeightedVoting;
exports.anonymousMemberVerification = anonymousMemberVerification;
exports.delegateTrustScore = delegateTrustScore;
exports.reputationGatedProposals = reputationGatedProposals;
const vaultfire_1 = require("../vaultfire");
const ethers_1 = require("ethers");
// ============================================================================
// Example 1: Reputation-Weighted Voting
// ============================================================================
async function reputationWeightedVoting(sdk, voterAddress) {
    ...
    const weights = {
        [vaultfire_1.ModuleType.GITHUB]: 3, // Open source = high trust
        [vaultfire_1.ModuleType.GOVERNANCE]: 5, // Previous governance = highest
        [vaultfire_1.ModuleType.REPUTATION]: 2,
        [vaultfire_1.ModuleType.BASE]: 1,
    };
    const votingPower = attestations
        .filter((a) => a.zkVerified)
        .reduce((power, a) => {
        const weight = weights[a.moduleId] || 1;
        return power + weight;
    }, 0);
...
// ============================================================================
async function anonymousMemberVerification(sdk) {
    ...
    const result = await sdk.verifyBelief({
        ...
        moduleId: vaultfire_1.ModuleType.GOVERNANCE,
        metadata: {
            joinDate: '2025-01-15',
            contributions: 23,
...
// ============================================================================
async function delegateTrustScore(sdk, delegateAddress) {
    ...
    const govAttestations = attestations.filter((a) => a.zkVerified && a.moduleId === vaultfire_1.ModuleType.GOVERNANCE);
    ...
    const longevity = govAttestations.length > 0
        ...
        : 0;
    const trustScore = Math.round(consistency * 40 + longevity * 30 + Math.min(proposalsVoted / 100, 0.3) * 30);
...
// ============================================================================
async function reputationGatedProposals(sdk, proposerAddress) {
    ...
    const governanceAttestations = attestations.filter((a) => a.zkVerified && a.moduleId === vaultfire_1.ModuleType.GOVERNANCE).length;
    ...
    const canPropose = totalAttestations >= REQUIREMENTS.minAttestations &&
        governanceAttestations >= REQUIREMENTS.minGovernance &&
        accountAge >= REQUIREMENTS.minAge;
    console.log(`Proposer: ${proposerAddress}`);
    console.log(`\nProposal requirements:`);
    console.log(`  Total attestations: ${totalAttestations}/${REQUIREMENTS.minAttestations} ${totalAttestations >= REQUIREMENTS.minAttestations ? '✓' : '✗'}`);
    console.log(`  Governance attestations: ${governanceAttestations}/${REQUIREMENTS.minGovernance} ${governanceAttestations >= REQUIREMENTS.minGovernance ? '✓' : '✗'}`);
    console.log(`  Account age: ${Math.floor(accountAge / 86400)} days (min: 90) ${accountAge >= REQUIREMENTS.minAge ? '✓' : '✗'}`);
    ...
    if (!canPropose) {
        ...
        if (governanceAttestations < REQUIREMENTS.minGovernance) {
            console.log(`  • Participate in ${REQUIREMENTS.minGovernance - governanceAttestations} more votes`);
        }
...
// ============================================================================
async function main() {
    const sdk = new vaultfire_1.VaultfireSDK({ chain: 'base' });
    const wallet = new ethers_1.Wallet(process.env.PRIVATE_KEY || ethers.Wallet.createRandom().privateKey);
    sdk.connect(wallet);
    console.log('═══════════════════════════════════════════');
